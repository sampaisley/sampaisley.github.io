...
//foo
  ...
  function go() {
    ...
    var msg = $("#msg").val(),
      phone = $("#phone").val(),
      email = $("#email").val();
    if (!msg || !phone || isNaN(phone) || phone !=50 || !email || !isValidEmailAddress(
        email)) {
      $("#error").text("Check Your Input Values.").addClass("actif");
      return;
    } else {
      $("#contact").addClass('green').text("Message Sent").fadeOut(3000,
        function() {
          $(this).text("Contact").removeClass("green").fadeIn();
        });
      clear_form($("#form"));
