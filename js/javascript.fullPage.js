...
(function(root, window, document, factory, undefined) {
  ...
  function init(callback) {
    ...
    for (var i = 0; i < sections.length; i++) {
      ...
      if (numSlides > 0) {
        ...
        var newHTML = '<div class="' + SLIDES_WRAPPER + '"><div class="' +
          SLIDES_CONTAINER + '">' + slidesHTML + '</div></div>';
  ...
  function afterDomReady() {
    ...
    for (var i = 0; i < slidesArrows.length; i++) {
      addListenerMulti(slidesArrows[i], 'click onclick touchstart',
        arrowsHandler);
    }
    ...
    if (activeSlide && (activeSectionIndex !== 0 || (activeSectionIndex ===
        0 && getNodeIndex(activeSlide) !== 0))) {
    ...
    if (destiny.length) {
      ...
      if (!options.animateAnchor && section.length) {
        ...
        isFunction(options.afterLoad) && options.afterLoad.call(section,
          destiny, (getNodeIndex(section) + 1));
    ...
    //support for IE > 8
    addHandler(document, scrollToAnchor, 'DOMContentLoaded',
  ...
  function getWindowWidth() {
    return 'innerWidth' in window ? window.innerWidth : document.documentElement
      .offsetWidth;
  }
  ...
  function setScrolling(element, val) {
    if (!options.autoScrolling || options.scrollBar || (element.self !=
        window && hasClass(element, SLIDES_WRAPPER))) {
  ...
  function reBuild(resizing) {
    ...
    isResizing = false;
    isFunction(options.afterResize) && resizing && options.afterResize.call(
      container);
    isFunction(options.afterReBuild) && !resizing && options.afterReBuild
      .call(container);
  }
  ...
  function afterRenderActions() {
    ...
    isFunction(options.afterLoad) && options.afterLoad.call(section,
      section.getAttribute('data-anchor'), (getNodeIndex(section) + 1));
    ...
    if (!options.autoScrolling || options.scrollBar) {
      ...
      if (!hasClass(currentSection, ACTIVE)) {
        ...
        if (canScroll) {
          ...
          isFunction(options.onLeave) && options.onLeave.call(
            leavingSection, leavingSectionIndex, sectionIndex,
            yMovement);
          isFunction(options.afterLoad) && options.afterLoad.call(
            currentSection, anchorLink, sectionIndex);
      ...
      if (options.fitToSection) {
        ...
        scrollId2 = setTimeout(function() {
          if (canScroll && !options.autoScrolling || options.scrollBar) {
            ...
            if (getNodeIndex($(SECTION_ACTIVE_SEL)) == getNodeIndex(
                currentSection)) {
              isResizing = true;
            }
  ...
  */
  function touchMoveHandler(event) {
    ...
    if (isReallyTouch(e)) {
      ...
      if (canScroll && !slideMoving) { //if theres any #
        ...
        if (slides && Math.abs(touchStartX - touchEndX) > (Math.abs(
            touchStartY - touchEndY))) {

          //is the movement greater than the minimum resistance to scroll?
          if (Math.abs(touchStartX - touchEndX) > (getWindowWidth() / 100 *
        ...
        //vertical scrolling (only when autoScrolling is enabled)
          ...
          //is the movement greater than the minimum resistance to scroll?
          if (Math.abs(touchStartY - touchEndY) > (getWindowHeight() /
              100 * options.touchSensitivity)) {
  ...
  function isReallyTouch(e) {
    ...
    return typeof e.pointerType === 'undefined' || e.pointerType !=
      'mouse';
  }
  ...
  function getAverage(elements, number) {
    ...
    var lastElements = elements.slice(Math.max(elements.length - number,
      1));
  ...
  function scrollPage(element, callback, isMovementUp) {
    ...
    if ((getNodeIndex(v.activeSection) == v.sectionIndex && !isResizing) ||
    ...
    isFunction(options.onLeave) && !v.localIsResizing && options.onLeave.call(
      v.activeSection, v.leavingSection, (v.sectionIndex + 1), v.yMovement
    );
  ...
  function afterSectionLoads(v) {
    ...
    isFunction(options.afterLoad) && !v.localIsResizing && options.afterLoad
  ...
  function hashChangeHandler() {
    if (!isScrolling) {
      ...
      if (section.length) {
        ...
        var isFirstSlideMove = (typeof lastScrolledDestiny ===
          'undefined');
        var isFirstScrollMove = (typeof lastScrolledDestiny ===
          'undefined' && typeof slide === 'undefined' && !slideMoving);

        /*in order to call scrollpage() only once for each destination at a time
        It is called twice for each scroll otherwise, as in case of using anchorlinks `hashChange`
        event is fired on every scroll too.*/
        if ((section && section !== lastScrolledDestiny) && !
          isFirstSlideMove || isFirstScrollMove || (!slideMoving &&
  ...
  document.onkeydown = function(e) {
    ...
    if (tagName !== 'SELECT' && tagName !== 'INPUT' && options.keyboardScrolling &&
  ...
  if (slidesNav !== null) {
    addListenerMulti($(SLIDES_NAV_LINK_SEL), 'click onclick touchstart',
      function(e) {
        preventDefault(e);
  ...
  function landscapeScroll(slides, destiny) {
    ...
    if (options.onSlideLeave) {
      ...
      if (!localIsResizing && xMovement !== 'none') {
        isFunction(options.onSlideLeave) && options.onSlideLeave.call(
          prevSlide, anchorLink, (sectionIndex + 1), prevSlideIndex,
          xMovement);
      }
    }
    ...
    var afterSlideLoads = function() {
      //if the site is not just resizing and readjusting the slides
      if (!localIsResizing) {
        isFunction(options.afterSlideLoad) && options.afterSlideLoad.call(
          destiny, anchorLink, (sectionIndex + 1), slideAnchor,
          slideIndex);
      }
    ...
    if (options.css3) {
      var translate3d = 'translate3d(-' + Math.round(destiny.offsetLeft) +
        'px, 0px, 0px)';
      ...
      scrollTo(slides, Math.round(destiny.offsetLeft), options.scrollingSpeed,
        function() {
          afterSlideLoads();
        });
    }
  ...
  function resizeHandler() {
    ...
    if (isTouchDevice) {
      ...
      if (document.activeElement.getAttribute('type') !== 'text') {
        ...
        if (Math.abs(currentHeight - previousHeight) > (20 * Math.max(
            previousHeight, currentHeight) / 100)) {
          reBuild(true);
  ...
  function getSlideByAnchor(slideAnchor, section) {
    var slides = $(SLIDES_WRAPPER_SEL, section);
    var slide = $(SLIDE_SEL + '[data-anchor="' + slideAnchor + '"]',
      slides);
  ...
  function setBodyClass(text) {
    ...
    var classRe = new RegExp('\\b\\s?' + VIEWING_PREFIX + '-[^\\s]+\\b',
      "g");
  ...
  function addMouseWheelHandler() {
    addHandler($(WRAPPER_SEL), MouseWheelHandler, 'mousewheel',
      'onmousewheel', 'wheel');
  }
  ...
  function addTouchHandler() {
    if (isTouchDevice || isTouch) {
      ...
      if (document.addEventListener) {
        ...
        addListenerMulti(wrapper, 'touchstart ' + MSPointer.down,
          touchStartHandler);
        addListenerMulti(wrapper, 'touchmove ' + MSPointer.move,
          touchMoveHandler);
      }
    }
  }
