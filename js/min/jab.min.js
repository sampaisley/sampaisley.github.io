'use strict';let jabber=document.getElementById("jabber"),container=document.getElementById("container"),score=document.getElementById("score"),sound1=new Audio("aaggh.mp3"),sound2=new Audio("aaggh2.mp3"),sound3=new Audio("aaggh3.mp3"),start=new Audio("start.mp3"),sounds=[sound2,sound3,sound1];function makeNewPosition(b,a){return[Math.floor(Math.random()*(b.clientHeight-a.offsetHeight))+b.offsetTop,Math.floor(Math.random()*(b.clientWidth-a.offsetWidth))+b.offsetLeft]}
function velocity(b,a){let c=Math.abs(b[1]-a[1]);b=Math.abs(b[0]-a[0]);return Math.ceil((c>b?c:b)/.3)}function floatArm(){let b=makeNewPosition(container,target),a=target.offsetTop,c=target.offsetLeft;target.animate([{top:a+"px",left:c+"px"},{top:b[0]+"px",left:b[1]+"px"}],{duration:velocity([a,c],b),easing:"ease-in-out",fill:"forwards"}).onfinish=function(){floatArm()}}let target=document.createElement("img");target.id="target";target.onload=function(){floatArm()};target.src="../img/arm2.png";container.appendChild(target);
