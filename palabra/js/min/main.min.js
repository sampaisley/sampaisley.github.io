...
for(let a=0;9>a;a++)kboardLow.innerHTML=0==a?kboardLow.innerHTML+`<div id='enter' data-l='${lowRow[a]}' class="padLong low">${lowRow[a]}</div>`:8==a?kboardLow.innerHTML+`<div id='back' data-l='${lowRow[a]}' class="padLong low">&#8592;</div>`:kboardLow.innerHTML+`<div data-l='${lowRow[a]}' class="pad low">${lowRow[a]}</div>`;kboardLow.innerHTML+="</div>";container.after(kboardTop);kboardTop.after(kboardMid);kboardMid.after(kboardLow);let box=0,start=0,end=start+5,boxes=document.querySelectorAll(".box");
const keys=(()=>{const a=[...Array(26)].map((b,c)=>String.fromCharCode(c+65));return a.concat(a.map(b=>b.toLowerCase()))})(),oneDay=864E5,firstDate=(new Date(" Feb 06 2022")).setHours(0,0,0,0),secondDate=(new Date).setHours(0,0,0,0),diffDays=Math.round(Math.abs((firstDate-secondDate)/oneDay));
...
function enterPressed(a){25<rowPad||(inputWord.length&&inputWord.length<wordLength?alert("Word too short"):!isInWordList()&&inputWord.length&&inputWord.length==wordLength?alert("Not in word list."):"Enter"==a&&0==box%wordLength&&box<=boxes.length&&enterKeyWorks&&(enterKeyWorks=!enterKeyWorks,start+=5,end+=5,checkWord()))}
...
let a=localStorage.getObj("storeScore");setTimeout(()=>alert(`Never mind dear, the word is ' ${theWord.join("").toUpperCase()}
    You scored: ${a.level}
