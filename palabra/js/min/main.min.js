...
function enterPressed(a){25<rowPad||(inputWord.length&&inputWord.length<wordLength?(alertz("Word too short",1200),shake()):!isInWordList()&&inputWord.length&&inputWord.length==wordLength?(alertz("Not in word list.",1200),shake()):"Enter"==a&&0==box%wordLength&&box<=boxes.length&&enterKeyWorks&&inputWord.length&&(enterKeyWorks=!enterKeyWorks,start+=5,localStorage.setItem("start",start),end+=5,localStorage.setItem("end",end),checkWord()))}
function checkWord(){copy=Array.from(theWord);triedWords.push(Array.from(inputWord));localStorage.setObj("triedWords",triedWords);inputWord.forEach((a,b,d)=>{a==theWord[b]?(greenKeys.push(inputWord[b]),savedClasses[b+rowPad]="green",localStorage.setObj("savedClasses",savedClasses),localStorage.setObj("greenKeys",greenKeys),copy[b]=null,inputWord[b]="*",lettersCorect++==wordLength-1&&winner()):(blackKeys.push(inputWord[b]),savedClasses[b+rowPad]="black",localStorage.setObj("savedClasses",savedClasses),
localStorage.setObj("blakKeys",blackKeys),lettersCorect=0)});orange();inputWord.length=0;rowPad+=5;localStorage.setItem("rowPad",rowPad);score.lastPlayed=(new Date).setHours(0,0,0,0);localStorage.setObj("storeScore",score);setKeyCoulors();lettersCorect<wordLength-1&&box>boxes.length-1&&(failed(),lettersCorect=0);setClasses()}
...
[];a=localStorage.getObj("savedClasses")||[];c=d=0;for(let e=0;e<boxes.length&&!(d>=b.length);)boxes[c++].innerHTML=b[d][e++],0==e%wordLength&&(d++,e=0);for(b=0;b<boxes.length;b++)boxes[b].classList.add(a[b])}$(".box").eq(29).css({cursor:"pointer"}).on("dblclick",function(){localStorage.clear();alertz("storage cleared.")});function shake(){$(".box").eq(start,inputWord.length).putClass("shake");setTimeout(()=>$(".box").eq(start,inputWord.length).takeClass("shake"),550)};