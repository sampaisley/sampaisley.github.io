...
let theWord=Array.from(wordList[diffDays]),copy=Array.from(theWord),wordLength=5,inputWord=[],triedWords=localStorage.getObj("triedWords")||[],lettersCorect=0,enterKeyWorks=!1,savedClasses=localStorage.getObj("savedClasses")||[],pads=document.querySelectorAll(".pad"),greenKeys=[],blackKeys=[],orangeKeys=[],greenBoxes=[],blackBoxes=[],orangeBoxes=[],score=localStorage.getObj("storeScore")||{gamesPlayed:0,level:0,pastLevels:[],lastPlayed:today,streak:1,maxStreak:1,gameOver:!1};
score.lastPlayed!=today?(triedWords=[],savedClasses=[],localStorage.setObj("triedWords",null),localStorage.setObj("savedClasses",null),localStorage.setObj("blakKeys",null),localStorage.setObj("greenKeys",null),localStorage.setObj("orangeKeys",null),localStorage.removeItem("box"),localStorage.removeItem("start"),localStorage.removeItem("end"),localStorage.removeItem("rowPad"),console.log(localStorage.getItem("rowPad")," <=====")):history();
let box=parseInt(localStorage.getItem("box"))||0,start=parseInt(localStorage.getItem("start"))||0,end=parseInt(localStorage.getItem("end"))||start+5,rowPad=parseInt(localStorage.getItem("rowPad"))||0;window.onkeydown=a=>{1!=score.gameOver&&("Enter"==a.key&&enterPressed(a.key),makeWord(a.key))};
function makeWord(a){console.log(box<end);if(-1!=keys.indexOf(a)&&box<end){if(29<box||lettersCorect>=wordLength)return;0==box%wordLength&&(enterKeyWorks=!0);boxes[box].classList.add("darkBorder");boxes[box++].innerHTML=a;localStorage.setItem("box",box);inputWord.push(a.toLowerCase())}"Backspace"==a&&box>start&&(inputWord.pop(),boxes[--box].innerHTML="",localStorage.setItem("box",box),boxes[box].classList.remove("darkBorder"))}
...
0;e<boxes.length&&!(d>=b.length);)boxes[c++].innerHTML=b[d][e++],0==e%wordLength&&(d++,e=0);for(b=0;b<boxes.length;b++)boxes[b].classList.add(a[b])}$(".box").eq(29).css({outline:"1px solid red"}).on("dblclick",function(){localStorage.clear();console.log("storage cleared.")});