...
kboardLow.innerHTML+="</div>";container.after(kboardTop);kboardTop.after(kboardMid);kboardMid.after(kboardLow);kboardLow.after(alertzDiv);let box=0,start=0,end=start+5,boxes=document.querySelectorAll(".box");const keys=(()=>{const a=[...Array(26)].map((b,c)=>String.fromCharCode(c+65));return a.concat(a.map(b=>b.toLowerCase()))})(),oneDay=864E5,firstDate=(new Date(" Feb 06 2022")).setHours(0,0,0,0),secondDate=(new Date).setHours(0,0,0,0),diffDays=Math.round(Math.abs((firstDate-secondDate)/oneDay));
...
window.addEventListener("click",a=>{a.target.hasAttribute("data-l")&&(letter=a.target.getAttribute("data-l"),makeWord(letter));"Enter"==letter&&enterPressed(letter);letter=null});let aalert=$("#alert");function alertz(a,b){aalert.takeClass("d-none").fadeUp(100).text(`<h1 class="alert">${a}</h1>`);a=$(".alert").eq(1).getRect().height;a=window.innerHeight/2-a/2-20;$(".alert").eq(1).css({"margin-top":`${a}px`});b&&setTimeout(closeAlertz,b)}
window.addEventListener("click",function(a){a.target.classList.contains("alert")&&closeAlertz()});function closeAlertz(){aalert.fade(50,!1,function(){aalert.putClass("d-none").text("")})}alertz(`Never mind dear, the word is ${theWord.join("").toUpperCase()}
    You scored: 44
    games played:44`);