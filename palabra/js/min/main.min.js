...
(function(){var a="qwertyuiop".split(""),b="asdfghjkl".split("");let d="Enter z x c v b n m Backspace".split(" "),c=document.getElementById("container");for(var e=0;30>e;e++)c.innerHTML+='<div class="box">&nbsp;</div>';e=document.createElement("div");e.id="alert";e.classList.add("alert");e.classList.add("d-none");const g=document.createElement("div");g.id="kBoardTop";const h=document.createElement("div");h.id="kBoardMid";const f=document.createElement("div");f.id="kBoardLow";g.innerHTML+='<div id="topRow">';
for(let k=0;10>k;k++)g.innerHTML+=`<div data-l='${a[k]}' class="pad top">${a[k]}</div>`;g.innerHTML+="</div>";h.innerHTML+='<div id="midRow">';for(a=0;9>a;a++)h.innerHTML+=`<div data-l='${b[a]}' class="pad mid">${b[a]}</div>`;h.innerHTML+="</div>";f.innerHTML+='<div id="lowRow">';for(b=0;9>b;b++)f.innerHTML=0==b?f.innerHTML+`<div id='enter' data-l='${d[b]}' class="padLong low">${d[b]}</div>`:8==b?f.innerHTML+`<div id='back' data-l='${d[b]}' class="padLong low">&#8592;</div>`:f.innerHTML+`<div data-l='${d[b]}' class="pad low">${d[b]}</div>`;
f.innerHTML+="</div>";c.after(g);g.after(h);h.after(f);f.after(e)})();let boxes=document.querySelectorAll(".box");const keys=(()=>{const a=[...Array(26)].map((b,d)=>String.fromCharCode(d+65));return a.concat(a.map(b=>b.toLowerCase()))})(),oneDay=864E5,firstDate=(new Date(" Feb 19 2022")).setHours(0,0,0,0),today=(new Date).setHours(0,0,0,0),diffDays=Math.round(Math.abs((firstDate-today)/oneDay));
let theWord=Array.from(wordList[diffDays]),copy=Array.from(theWord),wordLength=5,inputWord=[],triedWords=localStorage.getObj("triedWords")||[],lettersCorect=0,enterKeyWorks=!1,savedClasses=localStorage.getObj("savedClasses")||[],pads=document.querySelectorAll(".pad"),greenKeys=[],blackKeys=[],orangeKeys=[],score=localStorage.getObj("storeScore")||{gamesPlayed:0,level:0,pastLevels:[],lastPlayed:today,streak:1,maxStreak:1,gameOver:!1};
score.lastPlayed!=today?(triedWords=[],savedClasses=[],localStorage.setObj("triedWords",null),localStorage.setObj("savedClasses",null),localStorage.setObj("blakKeys",null),localStorage.setObj("greenKeys",null),localStorage.setObj("orangeKeys",null),localStorage.removeItem("box"),localStorage.removeItem("start"),localStorage.removeItem("end"),localStorage.removeItem("rowPad")):history();
let box=parseInt(localStorage.getItem("box"))||0,start=parseInt(localStorage.getItem("start"))||0,end=parseInt(localStorage.getItem("end"))||start+5,rowPad=parseInt(localStorage.getItem("rowPad"))||0;0<box&&0!=box+1%wordLength&&(box-=box%rowPad);window.onkeydown=a=>{1!=score.gameOver&&("Enter"==a.key&&enterPressed(a.key),makeWord(a.key))};
...
function checkWord(){copy=Array.from(theWord);triedWords.push(Array.from(inputWord));localStorage.setObj("triedWords",triedWords);inputWord.forEach((a,b,d)=>{inputWord[b]==theWord[b]?(greenKeys.push(inputWord[b]),savedClasses[b+rowPad]="green",localStorage.setObj("savedClasses",savedClasses),localStorage.setObj("greenKeys",greenKeys),boxes[b+rowPad].classList.add("green"),copy[b]=null,inputWord[b]="*",lettersCorect++==wordLength-1&&winner()):(boxes[b+rowPad].classList.add("black"),blackKeys.push(inputWord[b]),
savedClasses[b+rowPad]="black",localStorage.setObj("savedClasses",savedClasses),localStorage.setObj("blakKeys",blackKeys),lettersCorect=0)});orange();inputWord.length=0;rowPad+=5;score.lastPlayed=(new Date).setHours(0,0,0,0);localStorage.setItem("rowPad",rowPad);localStorage.setObj("storeScore",score);setKeyCoulors();lettersCorect<wordLength-1&&box>boxes.length-1&&failed()}
function orange(){for(let a=0;a<wordLength;a++){let b=copy.indexOf(inputWord[a]);-1!=b&&(boxes[a+rowPad].classList.add("yellow"),savedClasses[a+rowPad]="yellow",localStorage.setObj("savedClasses",savedClasses),orangeKeys.push(inputWord[a]),localStorage.setObj("orangeKeys",orangeKeys),copy[b]="]")}}function isInWordList(){let a=inputWord.join("");return wordList.includes(a)||Ta.includes(a)?!0:!1}
function winner(){score.gameOver=1;var a=score.lastPlayed;console.log("%c Math.round(Math.abs((lastGame - today) / oneDay)):","color: #0e93e0;background: #aaefe5;",Math.round(Math.abs((a-today)/oneDay)));1==Math.round(Math.abs((a-today)/oneDay))?(score.streak++,score.streak>=score.maxStreak&&(score.maxStreak=score.streak)):score.streak=1;score.gamesPlayed++;score.level=rowPad/wordLength+1;score.pastLevels.push(score.level);score.lastPlayed=(new Date).setHours(0,0,0,0);localStorage.setObj("storeScore",
score);a=localStorage.getObj("storeScore");alertz(`Well done dear!<br>
...
window.addEventListener("click",function(a){a.target.classList.contains("alert")&&closeAlertz()});function closeAlertz(){aalert.fade(50,!1,function(){aalert.putClass("d-none").text("")})}let boxClasses=[];score.lastPlayed<today?(score.gameOver=0,localStorage.setObj("storeScore",score)):history();
function history(){var a=localStorage.getObj("blakKeys"),b=localStorage.getObj("greenKeys"),d=localStorage.getObj("orangeKeys");for(var c=0;c<pads.length;c++)a&&-1!=a.indexOf(pads[c].getAttribute("data-l"))&&pads[c].classList.add("blackKey"),b&&-1!=b.indexOf(pads[c].getAttribute("data-l"))&&pads[c].classList.add("greenKey"),d&&-1!=d.indexOf(pads[c].getAttribute("data-l"))&&pads[c].classList.add("orangeKey");b=localStorage.getObj("triedWords")||[];a=localStorage.getObj("savedClasses")||[];c=d=0;for(let e=
0;e<boxes.length&&!(d>=b.length);)boxes[c++].innerHTML=b[d][e++],0==e%wordLength&&(d++,e=0);for(b=0;b<boxes.length;b++)boxes[b].classList.add(a[b])}$(".box").eq(29).css({outline:"1px solid green",cursor:"pointer"}).on("dblclick",function(){localStorage.clear();console.log("storage cleared.")});