...
f.id="kBoardLow";g.innerHTML+='<div id="topRow">';for(let k=0;10>k;k++)g.innerHTML+=`<div data-l='${a[k]}' class="pad top">${a[k]}</div>`;g.innerHTML+="</div>";h.innerHTML+='<div id="midRow">';for(a=0;9>a;a++)h.innerHTML+=`<div data-l='${b[a]}' class="pad mid">${b[a]}</div>`;h.innerHTML+="</div>";f.innerHTML+='<div id="lowRow">';for(b=0;9>b;b++)f.innerHTML=0==b?f.innerHTML+`<div id='enter' data-l='${d[b]}' class="padLong low"><img id="enterIcon"  data-l='${d[b]}' src="svg/enter.svg" /></div>`:8==
b?f.innerHTML+`<div id='back' data-l='${d[b]}' class="padLong low"><img id="backIcon"  data-l='${d[b]}' src="svg/back.svg" /></div>`:f.innerHTML+`<div data-l='${d[b]}' class="pad low">${d[b]}</div>`;f.innerHTML+="</div>";c.after(l);l.append(g);g.after(h);h.after(f);f.after(e)})();let boxes=document.querySelectorAll(".box");
const keys=(()=>{const a=[...Array(26)].map((b,d)=>String.fromCharCode(d+65));return a.concat(a.map(b=>b.toLowerCase()))})(),oneDay=864E5,firstDate=(new Date(" Feb 19 2022")).setHours(0,0,0,0),today=(new Date).setHours(0,0,0,0),diffDays=Math.round(Math.abs((firstDate-today)/oneDay));
...
savedClasses[b+rowPad]="black",localStorage.setObj("savedClasses",savedClasses),localStorage.setObj("blakKeys",blackKeys),lettersCorect=0)});orange();inputWord.length=0;rowPad+=5;localStorage.setItem("rowPad",rowPad);localStorage.setObj("storeScore",score);setKeyCoulors();lettersCorect<wordLength-1&&box>boxes.length-1&&failed()}
...
function failed(){score.gameOver=1;score.level=0;score.pastLevels.push(score.level);score.lastPlayed=(new Date).setHours(0,0,0,0);score.gamesPlayed++;localStorage.setObj("storeScore",score);let a=localStorage.getObj("storeScore");setTimeout(()=>alertz(`Never mind dear, the word is ${theWord.join("").toUpperCase()}
      <br>
      Games played: ${a.gamesPlayed}<br>Level: ${a.level}<br>Max Streak: ${a.maxStreak}<br> Level Distribution:<br>${getLevelDis(a.pastLevels)}`),1E3)}
...
0;e<boxes.length&&!(d>=b.length);)boxes[c++].innerHTML=b[d][e++],0==e%wordLength&&(d++,e=0);for(b=0;b<boxes.length;b++)boxes[b].classList.add(a[b])}$(".box").eq(29).css({cursor:"pointer"}).on("dblclick",function(){localStorage.clear();alertz("storage cleared.")});