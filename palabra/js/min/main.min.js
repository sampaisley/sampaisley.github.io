...
savedClasses),localStorage.setObj("blakKeys",blackKeys),lettersCorect=0)});orange();inputWord.length=0;rowPad+=5;localStorage.setItem("rowPad",rowPad);score.lastPlayed=(new Date).setHours(0,0,0,0);localStorage.setObj("storeScore",score);setKeyCoulors();lettersCorect<wordLength-1&&box>boxes.length-1&&failed();setClasses()}
...
for(let e=0;e<boxes.length&&!(d>=b.length);)boxes[c++].innerHTML=b[d][e++],0==e%wordLength&&(d++,e=0);for(b=0;b<boxes.length;b++)boxes[b].classList.add(a[b])}$(".box").eq(29).css({cursor:"pointer"}).on("dblclick",function(){localStorage.clear();alertz("storage cleared.")});